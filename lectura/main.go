...
func obtenerUsuarioPorId(respuesta http.ResponseWriter, peticion *http.Request) {
	...
	if err == nil {
		if idUsuarioBuscado < 2 {
			
			respuesta.Write([]byte("El id del usuario buscado es menor a 2"))
		} else{
			respuesta.Write([]byte("El id del usuario buscado es mayor o igual a 2"))
		}
	} else{
		respuesta.Write([]byte("No se pudo convertir la variable a int"))
	}
...
func main() {
	...
	enrutador.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
	...
	enrutador.HandleFunc("/cursos", func(respuesta http.ResponseWriter, peticion *http.Request) {
		...
		fmt.Fprintf(respuesta, "El orden: %s.\n", orden[0]) // Acceder al primer elemento
